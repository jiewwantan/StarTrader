...
# Reuse the training agent, StarTrader-v0 's preprocessed feature data
...
print("\n")
print("Base on non-correlation, {} stocks are selected for portfolio construction:".format(NUMBER_NON_CORR_STOCKS))
...
class StarTradingTestEnv(gym.Env):
    metadata = {'render.modes': ['human']}

    def __init__(self, day = START_TEST):
        ...
        # trading agent's action with low and high as the maximum number of stocks allowed to trade,
    ...
    def step(self, actions):
        ...
        self.done = self.day >= END_TEST

        # If it is the last step, plot trading performance
        if self.done:
            print("@@@@@@@@@@@@@@@@@")
            print("Iteration", self.iteration-1)
            # Construct trading and save to a spreadsheet for analysis
            ...
            trading_book.to_csv('./test_result/trading_book_test_{}.csv'.format(self.iteration-1))

            kpi = dp.MathCalc.calc_kpi(trading_book)
            kpi.to_csv('./test_result/kpi_test_{}.csv'.format(self.iteration-1))
            print("===============================================================================================")
            print(kpi)
            print("===============================================================================================")
            ...
            plt.title('Portfolio value + Account fund evolution @ test iteration {}'.format(self.iteration-1), fontsize=13)
            plt.legend()
            plt.savefig('./test_result/asset_evolution_test_{}.png'.format(self.iteration-1))
            plt.show()
            plt.close()
            ...
            plt.title('Cummulative returns @ test iteration {}'.format(self.iteration-1), fontsize=13)
            plt.legend()
            plt.savefig('./test_result/cummulative_returns_test_{}.png'.format(self.iteration-1))
            plt.show()
            plt.close()
            ...
            # Current state space
            self.state = [self.state[0]] + [self.unrealized_pnl] + self.data.values.tolist() + list(
                self.state[self.full_feature_length:])
